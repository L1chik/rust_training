...
impl State for Main {
    fn update(&mut self, client: &mut ChatClient) -> Result<Box<dyn State>, Error> {
        println!(
            "Select option:
        1) Join room
        2) Create room
...
impl State for Exit {
    fn update(&mut self, _: &mut ChatClient) -> Result<Box<dyn State>, Error> {
        unreachable!()
    }
...
impl State for JoinRoom {
    fn update(&mut self, chat: &mut ChatClient) -> Result<Box<dyn State>, Error> {
